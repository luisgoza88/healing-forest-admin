...
async function sendSingleWhatsAppMessage(recipient, message) {
    ...
    try {
        ...
        db.collection('whatsapp_queue').doc(docRef.id)
            .onSnapshot((doc) => {
                const data = doc.data();
                if (data.status === 'sent') {
                    ...
                    whatsappStats.pending--;
                    updateWhatsAppLog(recipient.phone, '❌ Falló: ' + (data.error || 'Error desconocido'), 'error');
                    updateWhatsAppStats();
                }
            });
    ...
    try {
        const response = await fetch(WHATSAPP_FUNCTION_URL, {
            ...
            body: JSON.stringify({
                ...
                message: '🧪 Test desde Cloud Functions - ' + new Date().toLocaleString(),
                type: 'test'
            })
        });
