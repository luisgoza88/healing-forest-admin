...
function initializeServiceCalendar(serviceId, containerId) {
    ...
    if (!calendarEl || !serviceConfig) {
        console.error('Invalid calendar container or service configuration');
        return;
    }
    ...
    const calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventDidMount: function(info) {
            ...
            // Add tooltip
            el.setAttribute('title', `${serviceConfig.name}\nCapacidad: ${booked}/${capacity}\nDisponible: ${available}`);
        },
        ...
        eventDrop: async function(info) {
            const confirmed = confirm(`¿Mover ${info.event.title} a ${info.event.start.toLocaleString('es')}?`);
        ...
        events: async function(fetchInfo, successCallback, failureCallback) {
            try {
                const events = await loadServiceEvents(serviceId, fetchInfo.start, fetchInfo.end);
...
function getBusinessHours(serviceId) {
    const hours = window.serviceCapacity.SERVICE_HOURS[serviceId] || window.serviceCapacity.SERVICE_HOURS.default;
    const businessHours = [];
...
async function loadServiceEvents(serviceId, startDate, endDate) {
    try {
        ...
        // Add blocked dates
        const blockedSnapshot = await db.collection('service_blocks')
            .where('serviceId', '==', serviceId)
            .where('date', '>=', firebase.firestore.Timestamp.fromDate(startDate))
            .where('date', '<=', firebase.firestore.Timestamp.fromDate(endDate))
            .get();
...
async function loadParticipants(serviceId, date, time) {
    try {
        ...
        const participantsSnapshot = await db.collection('appointments')
            .where('serviceId', '==', serviceId)
        ...
        if (participantsSnapshot.empty) {
            participantsList.innerHTML = '<p style="color: #666;">No hay participantes inscritos</p>';
            return;
        }
        ...
        document.getElementById('participantsList').innerHTML = '<p style="color: red;">Error al cargar participantes</p>';
    }
}
...
async function updateServiceEvent(event) {
    ...
    appointmentsSnapshot.forEach(doc => {
        ...
        console.log(`Notifying ${appointment.patientName} about schedule change`);
    });
}
...
function showServiceSettings(serviceId) {
    ...
    document.getElementById('serviceSettingsForm').addEventListener('submit', async (e) => {
        e.preventDefault();
        
        try {
            // Update capacity
            const newCapacity = parseInt(document.getElementById('serviceCapacity').value);
            await window.serviceCapacity.updateServiceCapacity(serviceId, newCapacity);
            ...
            for (const day of days) {
                ...
                if (openTime && closeTime) {
                    await window.serviceCapacity.updateServiceHours(serviceId, day, openTime, closeTime);
                }
            }
...
async function showServiceStats(serviceId) {
    ...
    try {
        ...
        const stats = await window.serviceCapacity.getServiceStatistics(serviceId, startDate, endDate);
        ...
        const content = `
            ...
            <ul>
                ${Object.entries(stats.peakHours).map(([hour, count]) => 
                    `<li>${hour}:00 - ${count} reservas</li>`
                ).join('')}
            </ul>
        ...
        showModal(`Estadísticas de ${serviceConfig.name} (Últimos 30 días)`, content);
...
function exportServiceSchedule(serviceId) {
    ...
    const data = events.map(event => ({
        ...
        Hora: event.start.toLocaleTimeString('es', { hour: '2-digit', minute: '2-digit' }),
    ...
    // Save file
    XLSX.writeFile(wb, `${serviceConfig.name}_horarios_${new Date().toISOString().split('T')[0]}.xlsx`);
}
...
function showAddParticipant(serviceId, date, time) {
    ...
    console.log('Add participant to', serviceId, date, time);
}
...
async function cancelServiceSlot(serviceId, date, time) {
    const confirmed = confirm('¿Estás seguro de cancelar esta clase? Se notificará a todos los participantes.');
    ...
    try {
        ...
        appointmentsSnapshot.forEach(doc => {
            ...
            console.log(`Notifying ${appointment.patientName} about class cancellation`);
        });
        
        alert('Clase cancelada exitosamente. Se ha notificado a todos los participantes.');
        closeModal();
        ...
        console.error('Error cancelling service slot:', error);
