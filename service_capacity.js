...
async function initializeServiceSchedules() {
  try {
    ...
    if (schedulesSnapshot.empty) {
      Logger.log('Initializing service schedules...');
    ...
    Logger.error('Error initializing service schedules:', error);
  }
}
...
async function updateServiceHours(serviceId, day, openTime, closeTime) {
  try {
    ...
    Logger.error('Error updating service hours:', error);
    return false;
  }
}
...
async function checkWaitingList(serviceId, date, time) {
  try {
    ...
    Logger.error('Error checking waiting list:', error);
    return null;
  }
}
...
async function notifyPatientFromWaitlist(waitingData) {
  try {
    ...
    if (waitingData.patientPhone) {
      ...
      Logger.log('Sending WhatsApp notification to:', waitingData.patientPhone);
    }

    // Send email notification
    if (waitingData.patientEmail) {
      // This would integrate with your email system
      Logger.log('Sending email notification to:', waitingData.patientEmail);
    }
  } catch (error) {
    Logger.error('Error notifying patient from waitlist:', error);
    throw error;
  }
}
...
async function blockServiceDates(serviceId, dates, reason = '') {
  try {
    ...
    Logger.error('Error blocking service dates:', error);
    return false;
  }
}
