...
function enhanceDateInputs() {
    const dateInputs = document.querySelectorAll('input[type="date"], input.date-picker, input[data-date-picker]');
    ...
    dateInputs.forEach(input => {
        ...
        const config = {
            ...FLATPICKR_CONFIG,
            dateFormat: "Y-m-d",
            minDate: input.hasAttribute('data-min-date') ? input.getAttribute('data-min-date') : "today",
            maxDate: input.hasAttribute('data-max-date') ? input.getAttribute('data-max-date') : null,
            disable: [
            ...
            onChange: function(selectedDates, dateStr, instance) {
                ...
                input.dispatchEvent(new CustomEvent('dateChanged', { 
...
function enhanceDateTimeInputs() {
    const datetimeInputs = document.querySelectorAll('input[type="datetime-local"], input.datetime-picker');
...
function enhanceTimeInputs() {
    const timeInputs = document.querySelectorAll('input[type="time"], input.time-picker');
...
function addQuickSelectButtons(instance) {
    ...
    buttonsContainer.style.cssText = "display: flex; gap: 5px; padding: 5px; border-top: 1px solid #e0e0e0;";
    
    const buttons = [
        { label: "Hoy", action: () => instance.setDate(new Date()) },
        { label: "Mañana", action: () => instance.setDate(moment().add(1, 'day').toDate()) },
        { label: "Próx. Semana", action: () => instance.setDate(moment().add(1, 'week').toDate()) },
        { label: "Próx. Mes", action: () => instance.setDate(moment().add(1, 'month').toDate()) }
    ];
    ...
    buttons.forEach(btn => {
        ...
        button.style.cssText = "flex: 1; background: #16A34A; color: white; border: none; padding: 5px; border-radius: 4px; font-size: 12px; cursor: pointer;";
...
function addHolidayIndicator(instance) {
    ...
    instance.config.onDayCreate = function(dObj, dStr, fp, dayElem) {
        ...
        if (holidays.includes(dateStr)) {
            dayElem.innerHTML += '<span class="holiday-indicator" title="Día festivo">🎉</span>';
...
function addTimePresets(instance) {
    ...
    presetsContainer.style.cssText = "display: flex; flex-wrap: wrap; gap: 5px; padding: 5px; border-top: 1px solid #e0e0e0;";
    
    const commonTimes = ["08:00", "09:00", "10:00", "11:00", "14:00", "15:00", "16:00", "17:00"];
    
    commonTimes.forEach(time => {
        const button = document.createElement("button");
        button.innerHTML = time;
        button.style.cssText = "background: #f3f4f6; border: 1px solid #d1d5db; padding: 3px 8px; border-radius: 4px; font-size: 12px; cursor: pointer;";
        button.addEventListener("click", function(e) {
            e.preventDefault();
...
function addCommonTimeSlots(instance) {
    ...
    slotsContainer.style.cssText = "display: grid; grid-template-columns: repeat(4, 1fr); gap: 5px; padding: 5px;";
    ...
    slots.forEach(time => {
        const button = document.createElement("button");
        button.innerHTML = time;
        button.style.cssText = "background: #f3f4f6; border: 1px solid #d1d5db; padding: 5px; border-radius: 4px; font-size: 12px; cursor: pointer;";
        button.addEventListener("click", function(e) {
            e.preventDefault();
...
function enhanceReportFilters() {
    const reportContainers = document.querySelectorAll('.report-filters, .reports-section, [data-report-filter]');
        ...
        mutations.forEach((mutation) => {
            mutation.addedNodes.forEach((node) => {
                if (node.nodeType === 1) { // Element node
                    ...
                    if (node.matches && node.matches('input[type="date"], input[type="time"], input[type="datetime-local"]')) {
...
function enhanceAllCalendars() {
    ...
    const calendarContainers = document.querySelectorAll('[id*="calendar"], .calendar-container, [data-calendar]');
    ...
    const calendar = new FullCalendar.Calendar(container, {
        ...
        resourceLabelDidMount: function(info) {
            ...
            if (resource.extendedProps.capacity) {
                ...
                capacityBadge.style.cssText = 'margin-left: 10px; font-size: 0.8em; color: #666;';
                info.el.querySelector('.fc-datagrid-cell-main').appendChild(capacityBadge);
            }
        },
...
function enhanceEventDisplay(info, serviceConfig) {
    ...
    if (serviceConfig) {
        ...
        badge.style.cssText = 'position: absolute; top: 2px; right: 2px; background: rgba(0,0,0,0.5); color: white; font-size: 10px; padding: 2px 4px; border-radius: 3px;';
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                ...
                // Refresh calendar
                const calendar = document.querySelector(`[data-service-id="${serviceId}"]`)?._fcCalendar;
                if (calendar) {
...
function updateAvailableSlots(date) {
    ...
    console.log('Updating available slots for date:', date);
}
        ...
        mutations.forEach((mutation) => {
            mutation.addedNodes.forEach((node) => {
                if (node.nodeType === 1) {
                    ...
                    if (node.id === 'calendarModal' || node.classList?.contains('calendar-modal')) {
                        setTimeout(() => {
...
function initializeModalCalendar(modal) {
    const calendarContainer = modal.querySelector('#serviceCalendar, [data-calendar-container]');
    if (calendarContainer && !calendarContainer._fcCalendar) {
        const serviceId = modal.dataset.serviceId || calendarContainer.dataset.serviceId;
...
window.quickBookAppointment = async function(serviceId) {
    ...
    try {
        ...
        // Refresh calendar
        const calendar = document.querySelector('.calendar-modal .calendar-container')?._fcCalendar;
        if (calendar) {
            calendar.refetchEvents();
        }
...
window.showServiceCalendar = function(serviceId) {
    ...
    const serviceName = serviceCard?.querySelector('h3')?.textContent || 'Servicio';
