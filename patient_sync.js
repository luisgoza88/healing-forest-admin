    ...
    try {
        ...
        if (snapshot.empty) {
            ...
            snapshot.forEach(doc => {
                ...
                const lastVisit = lastAppointment ? new Date(lastAppointment.date.seconds * 1000).toLocaleDateString() : 'Nunca';
...
async function viewPatientDetails(patientId) {
    try {
        ...
        appointmentsSnapshot.forEach(doc => {
            const apt = doc.data();
            const date = apt.date ? new Date(apt.date.seconds * 1000).toLocaleDateString() : 'N/A';
            const time = apt.startTime || apt.time || 'N/A';
            const status = apt.status || 'pending';
            const statusColor = status === 'completed' ? 'green' : status === 'confirmed' ? 'blue' : 'orange';
...
_Para no recibir más mensajes, responde STOP_`;
    ...
    db.collection('whatsapp_queue').add({
        to: phone,
        message: message,
        type: 'direct',
        timestamp: new Date(),
        ...
        .onSnapshot((snapshot) => {
            snapshot.docChanges().forEach((change) => {
                if (change.type === 'added') {
                    console.log('New patient registered:', change.doc.data());
                    // Show notification
                    showNotification('Nuevo Paciente', `${change.doc.data().name || 'Usuario'} se ha registrado`);
                }
                
                if (change.type === 'modified') {
                    console.log('Patient updated:', change.doc.data());
        ...
        .onSnapshot((snapshot) => {
            snapshot.docChanges().forEach((change) => {
                if (change.type === 'added') {
                    ...
                    console.log('New appointment created:', apt);
                    showNotification('Nueva Cita', `Cita agendada para ${apt.serviceName || 'Servicio'}`);
...
console.log('Patient sync module loaded - Real-time synchronization active');