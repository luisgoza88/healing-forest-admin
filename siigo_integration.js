...
class SiigoIntegration {
  ...
  async testConnection() {
    try {
      ...
      Logger.error('Siigo connection error:', error);
      return false;
    }
  }
      ...
      contacts: [
        {
          ...
          last_name:
            patientData.lastName || patientData.name.split(' ')[1] || '',
  ...
  async sendInvoiceByEmail(invoiceId, email) {
    try {
      const response = await fetch(
        `${this.baseURL}/invoices/${invoiceId}/mail`,
        {
          method: 'POST',
          headers: this.headers,
          body: JSON.stringify({
...
const siigoWorkflow = {
  ...
  async syncAllServices() {
    ...
    const config = (
      await db.collection('integrations').doc('siigo').get()
    ).data();
    const siigo = new SiigoIntegration(config);
