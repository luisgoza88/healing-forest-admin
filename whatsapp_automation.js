...
class WhatsAppAutomation {
    ...
    async scheduleMessage(to, message, sendAt) {
        ...
        const docRef = await db.collection('scheduled_messages').add(scheduledMessage);
        return docRef.id;
    }
...
const whatsappWorkflows = {
    ...
    async setupAppointmentReminders() {
        ...
        for (const doc of appointments.docs) {
            ...
            try {
                ...
                results.push({ appointmentId: doc.id, success: false, error: error.message });
            }
        }
    ...
    async processScheduledMessages() {
        ...
        const messages = await db.collection('scheduled_messages')
            .where('sendAt', '<=', now)
